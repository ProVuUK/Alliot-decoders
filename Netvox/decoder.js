...
function Decoder(bytes, fport) {
    ...
    if (fport === 6) { // then its ReportDataCmd
        ...
        } else if ((bytes[1] === 0xA5) && (bytes[2] === 0x01)) { // device type A5 (R720E), report type 01
            decoded.devicetype = "R720E";
            decoded.battery = bytes[3] / 10;
            decoded.TVOC = ((bytes[4] << 8) + bytes[5]);
            decoded.temperature = ((bytes[6] << 8) + bytes[7]) / 100;
            decoded.humidity = ((bytes[8] << 8) + bytes[9]) / 100;
