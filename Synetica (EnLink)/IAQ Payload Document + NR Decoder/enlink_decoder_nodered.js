...
//msg = {};
//msg.eui = '0004a30b000478e1';
//msg.payload = Buffer.from(process.argv[2], 'hex');

if (!msg.payload.endDevice.devEui)
	return null;

// --------------------------------------------------------------------------------------
// Ignore Port 0 Possible MAC Command
// If there is no payload, there is no need for a port, thus it equals zero
if (msg.payload.fPort === 0) {
	if (msg.eui) {
...
if (msg.payload.payload) {
	if (msg.payload.payload.length === 0) {
		if (msg.payload.endDevice.devEui) {
			node.warn("Zero-length Payload, message ignored from " + msg.payload.endDevice.devEui);
		} else {
	...
	if (msg.payload.endDevice.devEui) {
		node.warn("No Payload, message ignored from " + msg.payload.endDevice.devEui);
	} else {
...
const TYPE_TEMP_PROBE_IN_BAND_ALARM_COUNT_1 = 0x1D; /* Number of times in band alarm has been activated for temperature probe 1  */
const TYPE_TEMP_PROBE_IN_BAND_ALARM_COUNT_2 = 0x1E; /* Number of times in band alarm has been activated for temperature probe 2 */
const TYPE_TEMP_PROBE_IN_BAND_ALARM_COUNT_3 = 0x1F; /* Number of times in band alarm has been activated for temperature probe 3 */
...
const TYPE_TEMP_PROBE_LOW_ALARM_COUNT_1 = 0x23; /* Number of times low threshold alarm has been activated for temperature probe 1  */
const TYPE_TEMP_PROBE_LOW_ALARM_COUNT_2 = 0x24; /* Number of times low threshold alarm has been activated for temperature probe 2 */
const TYPE_TEMP_PROBE_LOW_ALARM_COUNT_3 = 0x25; /* Number of times low threshold alarm has been activated for temperature probe 3 */
...
const TYPE_TEMP_PROBE_HIGH_ALARM_COUNT_1 = 0x29; /* Number of times high threshold alarm has been activated for temperature probe 1  */
const TYPE_TEMP_PROBE_HIGH_ALARM_COUNT_2 = 0x2A; /* Number of times high threshold alarm has been activated for temperature probe 2 */
const TYPE_TEMP_PROBE_HIGH_ALARM_COUNT_3 = 0x2B; /* Number of times high threshold alarm has been activated for temperature probe 3 */
...
function DecodePayload(data) {
	var obj = {};
	obj.devEui = msg.payload.endDevice.devEui;
	var msg_ok = false;
...
var res = DecodePayload(Buffer.from(msg.payload.payload, 'hex'));
if (res !== null) {
