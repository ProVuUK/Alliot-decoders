function decodeUplink(input, fport) {
    ...
    var output = 0;
    var data =[];
    var stringHex = bytesString(input);
    ...
    function dataOutput(octetTypeMessage) {
        var outputTypeMessage = [productStatusDataOutput(stringHex), productConfigurationDataOutput(stringHex), smokeAlarmDataOutput(stringHex), dailyAirDataOutput(stringHex), realTimeDataOutput(stringHex), temperatureDatalogDataOutput(stringHex)]
    ...
    function productConfigurationDataOutput(stringHex) {
        ...
        var data_nb_new_data = (parseInt(stringHex.substring(6, 10), 16) >> 3) & 0x3F;
        ...
        for(var i = 0; i < mesure.length; i++){
            data[i] = {
                ...
                fields:{
                    ...
                    "temperature": mesure[i],
                    "tempUnit": "°C",
                    "timestamp": msg.payload.recvTime - (data_time_between_measurement_sec * 60) * 1000 * (i + 1)
                    },
    ...
    data = temperatureDatalogDataOutput(stringHex);
    return data;

}
try {
    var tempPayload = decodeUplink(Buffer.from(msg.payload.payload, 'hex'), msg.payload.fPort);
